...
# Supplementary Fig. 1 | CAPEX overview
...
salesPbarPlots <- ggplot(salesPricedata, aes(x = vehicleParameterScenario)) +
  ...
  plotTheme(baseTextSize) +
    theme(
      ...
      plot.margin   =  margin(t = 3, r = 1, b = 3, l = 1))
```
...
FCPrices <- ggplot(
  ...
  geom_ribbon(
    ...
    alpha = 0.5 ) +
  geom_line(
    aes(x = period, y = MC_MTM, color = parameter), linewidth = baseLineWidth
  ) +
  ...
  labs(x = "", y = expression("Cost [EUR2020 kW"^-1*"]"), title = "Fuell cell\nsystem") +
  plotTheme(baseTextSize) +
    theme(
      panel.grid.major.x = element_blank(),
      panel.grid.minor.x = element_blank(),
...
BatPrices <- ggplot(
  ...
  scale_x_continuous(breaks = seq(2030, 2050, 10), expand = c(0, 0)) +
  scale_y_continuous(limits = c(0, 230),breaks = seq(0, 230, 50), expand = c(0, 0))+
  labs(x = "", y = expression("Costs [EUR2020 kWh"^-1*"]"), title = "Battery\nsystem" )+
  plotTheme(baseTextSize) +
    theme(
      panel.grid.major.x = element_blank(),
      panel.grid.minor.x = element_blank(),
...
ggsave(
  file.path(plotFolder, "SupFig1.pdf"),
  plot     = combinedCAPEX,
  device   = cairo_pdf,
  width  = 7.1,          
  height = 3.94,          
...
# Supplementary Fig. 3 | Shares of bio, fossil, and synthetic diesel in the diesel fuel blend at the gas station
...
facetNames <- unique(dieselBlendShares$energyCarrierParameterScenario)
tagData <- data.frame(
  energyCarrierParameterScenario = facetNames,
  label = c("a", "b"), 
  x = -Inf,            
  y = Inf              
)

combinedBlend <- ggplot(dieselBlendShares[period < 2045], aes(x = period, y = value, fill = parameter)) +
  geom_area(position = "stack", linewidth = 0, alpha = 0.8) +
  ...
  theme(
    panel.spacing = unit(2, "lines"),  # Give slightly more room for the tags between facets
    panel.border = element_rect(color = "grey85", fill = NA, linewidth = 0.5 * baseLineWidth),
    plot.margin = margin(t = 15, r = 5, b = 5, l = 15) # Leave margin space at top and left for tags
  ) 
...
ggsave(
  file.path(plotFolder, "SupFig3.pdf"),
  plot = combinedBlend,
  device   = cairo_pdf,
  width  = 7.1,          
  height = 2.37,         
...
# Supplementary Fig. 4 | Enduse energy carrier prices overview.
...
HydrogenPrices <- ggplot(
  ...
  scale_x_continuous(breaks = seq(2030, 2050, 10), expand = c(0, 0)) +
  labs(x = "Year", y = expression("Hydrogen [EUR2020 kWh"^-1*"]"), title = "Wholesale vs.\nenduse", fill = "") +
    guides(
...
ElecPrices <- ggplot(
  ...
  scale_x_continuous(breaks = seq(2030, 2050, 10), expand = c(0, 0)) +
  labs(x = "Year", y = expression("Electricity [EUR2020 kWh"^-1*"]"), title = "Wholesale vs.\nenduse", fill = "") +
    guides(
    fill = guide_legend(override.aes = list(pattern = "none")),
    pattern = guide_legend(title = "parameter")
  ) +
...
DieselPrices <- ggplot() +
  ...
  coord_cartesian(ylim = c(0, 0.3)) +
  scale_x_continuous(breaks = seq(2030, 2050, 10), expand = c(0, 0)) +
  labs(x = "Year", y = expression("Diesel [EUR2020 kWh"^-1*"]"), title = "Wholesale vs.\nenduse", fill = "") +
    guides(
    fill = guide_legend(override.aes = list(pattern = "none")),
    pattern = guide_legend(title = "parameter")
  ) +
...
hydrogenBarPlots <- ggplot(hydrogen, aes(x = `energyCarrierParameterScenario`)) +
 ...
 scale_fill_manual(values = rose5) + 
  labs(x = "", y = expression("Hydrogen [EUR2020 kWh"^-1*"]"), title = "Hydrogen: Enduse price\ncomponents (2030)", fill = "") +
  scale_x_discrete(labels = function(x) { sub("\\s", "\n", x) }, expand = expansion(mult = c(0.3, 0.3))) +
  plotTheme(baseTextSize) +
  theme(
...
electricityBarPlots <- ggplot(electricity, aes(x = `energyCarrierParameterScenario`)) +
 ...
 scale_fill_manual(values = purple7) + 
  ...
  labs(x = "", y = expression("Electricity [EUR2020 kWh"^-1*"]"), title = "Electricity: Enduse price\ncomponents (2030)", fill = "") +
  scale_x_discrete(labels = function(x) { sub("\\s", "\n", x) }, expand = expansion(mult = c(0.3, 0.3))) +
  plotTheme(baseTextSize) +
  theme(
...
dieselBarPlots <- ggplot(diesel, aes(x = `energyCarrierParameterScenario`)) +
 ...
 scale_fill_manual(values = green4) + 
  facet_wrap(~ energyCarrier, nrow = 1) +
  labs(x = "", y = expression("Diesel [EUR2020 kWh"^-1*"]"), title = "Diesel: Enduse price\ncomponents (2030)", fill = "") +
  scale_x_discrete(labels = function(x) { sub("\\s", "\n", x) }, expand = expansion(mult = c(0.3, 0.3))) +
  plotTheme(baseTextSize) +
  theme(
    panel.spacing = unit(0.2, "lines"), 
...
ggsave(
  file.path(plotFolder, "SupFig4.pdf"),
  plot = combined,
  device   = cairo_pdf,
  width  = 7.1,          
  height = 9.47,        
...
# Supplementary Fig. 5 | Sensitivity analysis regarding resale value, interest rate and vehicle insurance
```{r, fig.width= 4.5, fig.height= 2.5, fig.fullwidth=FALSE}
paperScenario <- c("Sensitivity | Current Policies | Resale value", "Sensitivity | Current Policies | Interest rate", "Sensitivity | Current Policies | Vehicle insurance", "Current policies")
...
DCOmilageDistributionShares <- calculateFreightActivityShares(DCOmilageDistribution, focus = "EUR")
  ...
  data <- copy(DCOmilageDistributionShares)[paperScen %in% paperScenario & period < 2041 & DCOscenario == "Medium"]
  ...
  findZeroPoints[, DCOscenario := factor(DCOscenario, levels = c("Medium"))]
  
  p1 <- ggplot(findZeroPoints[paperScen %in% c("Current policies", "Sensitivity | Current Policies | Resale value")], 
  ...
  p2 <- ggplot(findZeroPoints[paperScen %in% c("Current policies", "Sensitivity | Current Policies | Interest rate")]) +
    ...
    scale_linetype_manual(values = paperLines, labels = c("Current policies",
               "Sensitivity case")) +
    scale_x_continuous(breaks = seq(min(findZeroPoints$period),
                                    max(findZeroPoints$period), by = 5)) +
    scale_y_continuous(limits = c(0, 102)) +
  ...
  p3 <- ggplot(findZeroPoints[paperScen %in% c("Current policies", "Sensitivity | Current Policies | Vehicle insurance")], 
               aes(x = period, y = cumBinWeightedShareEUR * 100, color = truckTechnology, linetype = paperScen)) +
    geom_line(linewidth = baseLineWidth) +
    scale_color_manual(values = paperColors, guide = "none") +  # legend removed
    scale_linetype_manual(values = paperLines, guide = "none") +
  ...
  combined <- (
    (p1 | p2 | p3) +
      ...
      theme(
        plot.tag = element_text(face = "bold", size = baseTextSize*1.2),
...
plotWithLabels <- ggdraw(combined) +
  draw_label(
    "Sensitivity |\nReduce resale value\nof ZETs from 25 % to 10 %", x = 0.125, y = 0.93, angle = 0,
    fontface = "bold",
    fontfamily = "sans",
    size = baseTextSize * 1,
    hjust = 0
  ...
  draw_label(
    "Sensitivity |\nIncrease interest rate\nfor ZETs from 9.5 % to 11 %", x = 0.44, y = 0.93, angle = 0,
    fontface = "bold",
    fontfamily = "sans",
    size = baseTextSize * 1,
    hjust = 0
  ...
  draw_label(
    "Sensitivity |\nIncluding vehicle\ninsurance of 5.8% VPP/yr", x = 0.75, y = 0.93, angle = 0,
    fontface = "bold",
    fontfamily = "sans",
    size = baseTextSize * 1,
    hjust = 0
...
ggsave(
  filename = file.path(plotFolder, paste0("SupFig5.pdf")),
  plot     = plotWithLabels,
  device   = cairo_pdf,
  width  = 7.1,          
  height = 3.63,          
...
# SupplementaryFig. 6 | Country-specific breakeven annual mileages
...
data <- copy(breakeven)[paperScen == paperScenario & DCOscenario == DCOscen & country != "EUR"]
  
  p1 <- ggplot(data[period == 2030], 
               aes(x = country, y = AMb, color = truckTechnology, shape = truckClass)) +
    geom_point() +
    scale_shape_manual(values = c("Rigid" = 17, "Tractor-trailer" = 15)) +
    scale_color_manual(values = paperColors, guide = "none") +  # legend removed
    labs(y = "Breakeven\nannual mileage [vehkm/yr]", x = NULL) +
...
ggsave(
  filename = file.path(plotFolder, paste0("SupFig6.pdf")),
  plot     = plotWithLabels,
  device   = cairo_pdf,
  width  = 7.1,          
  height = 3.63,         
...
# Supplementary Fig.7 | Country-specific economically viable activity
```{r, fig.width= 4.5, fig.height= 2.3, fig.fullwidth=FALSE}
paperScenario <- "Current policies"
DCOscen <- "Medium"
...
setnames(mileageDistribution, "binMean", "annualM")
DCOmilageDistribution <- applyMileage(mileageDistribution, DCOdata)
DCOmilageDistributionShares <- calculateFreightActivityShares(DCOmilageDistribution, focus = "Country")

data <- copy(DCOmilageDistributionShares)[paperScen == paperScenario & DCOscenario == DCOscen]
  
findZeroPoints <- data[ , .SD[which.min(abs(value))], by = .(period, truckTechnology, DCOscenario, country)][, .(period, `truckTechnology`, DCOscenario, country, cumBinShareCountry)]
findZeroPoints[, DCOscenario := factor(DCOscenario, levels = c("Medium"))]
  
  p1 <- ggplot(findZeroPoints[period == 2030], 
               aes(x = country, y = cumBinShareCountry * 100, color = truckTechnology)) +
    geom_point() +
    scale_color_manual(values = paperColors, guide = "none") +  # legend removed
    labs(y = "Economically viable\nactivity [%]", x = NULL) +
  ...
  p2 <- ggplot(findZeroPoints[period == 2035], 
               aes(x = country, y = cumBinShareCountry * 100, color = truckTechnology)) +
    geom_point() +
    scale_color_manual(values = paperColors) +  
    coord_cartesian(ylim = c(0, 102)) +
  ...
  p3 <- ggplot(findZeroPoints[period == 2040], 
               aes(x = country, y = cumBinShareCountry * 100, color = truckTechnology)) +
    geom_point() +
    scale_color_manual(values = paperColors, guide = "none") +  # legend removed
    labs(x = NULL, y = NULL) +
...
ggsave(
  filename = file.path(plotFolder, paste0("SupFig7.pdf")),
  plot     = plotWithLabels,
  device   = cairo_pdf,
  width  = 7.1,          
  height = 3.63,          
...
# Supplementary Fig.9 | Heterogeneous DCO of BETs against ICETs for feasible maximum daily mileages, and their respective cumulative share of vehicles.
...
ggsave(
  filename = file.path(plotFolder, paste0("SupFig9.pdf")),
  plot     = plotWithLabels,
  device   = cairo_pdf,
  width  = 7.1,          
  height = 3.26,,          
...
# Supplementary Figure 10 | Economically viable road freight activity for the optimistic, medium, and pessimistic differential cost of ownership (DCO) scenario comparing battery electric trucks (BETs) with fuel cell electric trucks (FCETs).
...
## Combine meta and mileage distribution plot
...
TCOxAnnualMileagePlotNoleg <- TCOxAnnualMileage + theme(legend.position = "none")
metaPlotNoLegend <- MetaTCOxAnnualMileage + theme(legend.position = "none")


combined <- wrap_plots(TCOxAnnualMileagePlotNoleg) + 
            plot_spacer() + 
            wrap_plots(metaPlotNoLegend) + 
            plot_layout(widths = c(1.2, 0.05, 0.8), ncol = 3) +
            plot_annotation(tag_levels = 'a') & 
            theme(
                plot.tag = element_text(face = "bold", size = baseTextSize*1.2),
                plot.tag.position = c(-0.05, 1.02), 
                plot.margin = margin(t = 8, r = 3, b = 3, l = 15)
              )
...
ggsave(
  filename = file.path(plotFolder, paste0("SupFig10.pdf")),
  plot     = plotWithLabels,
  device   = cairo_pdf,
  width  = 7.1,          
  height = 8.68,          
...
# Supplementary Figure 11 | Assumptions on evolution of fast-charging infrastructure availability over time.
```{r, fig.width=3, fig.height=1.5, fig.fullwidth=FALSE}
infrastructure <- loadInfrastructureBuildUp(dataFolder)

infraPlot <- ggplot(data = infrastructure[period >= 2025], aes(x = period, y = value * 100)) +
  geom_line(linewidth = baseLineWidth) +
...
ggsave(
  file.path(plotFolder, paste0("SupFig11.pdf")),
  plot = infraPlot,
  device   = cairo_pdf,
  width    = 3,          
  height   = 1.5,          
