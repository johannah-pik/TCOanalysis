...
FeasTCOxAnnualMileage2<- amDistributionBarPlot(feasDCO[DCOscenario == DCOscen], yr = 2040, paperScenario, feas = TRUE)
...
DCOmilageDistributionShares <- calculateFreightActivityShares(DCOmilageDistribution, focus = "EUR")
  ...
  arrow <- dcast(arrow, period + truckTechnology + DCOscenario ~ paperScen)
  arrow <- arrow[
  ...
  p1 <- ggplot(findZeroPoints[DCOscenario == "Optimistic"], 
    ...
    labs(y = "Economically viable road\nfreight activity [%]", x = NULL) +
  ...
  combined <- (
    (p1 | p2 | p3) +
      ...
      theme(
        legend.position = "bottom",
        plot.margin   =  margin(t = 10, r = 3, b = -2, l = 3)
      ))
  plot(combined)
    
finalPlot_with_labels <- ggdraw(combined) +
  draw_label(
    "Optimistic", x = 0.17, y = 0.93, angle = 0,
    fontface = "bold",
    fontfamily = "sans",
    size = baseTextSize * 1,
    hjust = 0
  ...
  draw_label(
    "Pessimistic", x = 0.82, y = 0.93, angle = 0,
    fontface = "bold",
    fontfamily = "sans",
    size = baseTextSize * 1,
    hjust = 0
...
ggsave(
  ...
  device   = cairo_pdf,
  width    = 4.5,          # in inches ≈ 180 mm / 25.4
  height   = 2.3,          # in inches (adjust aspect ratio as appropriate)
  units    = "in",
  dpi      = 300           # ensures minimum resolution for any raster elements
)
```
